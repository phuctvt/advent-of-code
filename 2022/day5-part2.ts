...
class Stack {
    ...
    /**
     * First item will be pushed in first and so on.
     */
    pushMultiple(crates: Crate[]) {
        this.crates.push(...crates);
    }

    pop(size: number = 1): Crate[] {
        const result = this.crates.slice(this.count - size);
        this.crates.splice(this.count - size, size);
        return result;
    }
...
export function getResult(): string {
    ...
    for (const step of procedure) {
        const crates = step.from.pop(step.quantity);
        step.to.pushMultiple(crates);
    }
